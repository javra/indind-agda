...
U {Γ} = record { ᴬ   = λ γ → Set ;
                 ...
                 t   = λ γ γᴬ δ ρ α αᴬ αʷ αᴿ → (xᴱ : α) → Σ αᴬ λ xᴬ → αᴿ xᴬ xᴱ ;
                 sg  = λ γ δ α ω → Σ α ω ;
                 m   = λ { γ γᴬ δ ρ ϕ τ α αᴬ αʷ αᴿ αᶠ αᵗ (xᴱ , xʷ)
                         → ₁ (αᵗ xᴱ) } }
  where
    module Γ = Con Γ
...
El {Γ} a = record { ᴬ   = λ γᴬ → a.ᴬ γᴬ ;
                    ...
                    m   = λ γ γᴬ δ ρ ϕ τ α αᴬ αʷ αᴿ → lift {!!} }
  where
    module Γ = Con Γ
    module a = TmS a
...
ΠS {Γ} a B = record { ᴬ   = λ γᴬ → (α : a.ᴬ γᴬ) → B.ᴬ (γᴬ , α) ;
                      ᴹ   = λ {γᴬ} γᴹ πᴬ ϕᴬ → (αᴬ : a.ᴬ γᴬ) → B.ᴹ (γᴹ , lift {!!}) (πᴬ αᴬ) (ϕᴬ (a.ᴹ γᴹ αᴬ)) ;
                      ...
                      m   = λ {γc} γ γᴬ {δc} δ {ρc} ρ ϕ τ π πᴬ πʷ πᴿ πᶠ πᵗ αᴬ
                              → let xᴱ = ₁ (coe (a.sg γ δ) αᴬ) in
                                let xᴬ = ₁ (a.t γ γᴬ δ ρ xᴱ) in
                                let xʷ = ₂ (coe (a.sg γ δ) αᴬ) in
                                let xᴿ = ₂ (a.t γ γᴬ δ ρ xᴱ) in
                                {!!} {- coe {!!}
...
σ ∘ δ = record { ᴬ   = λ γ → σ.ᴬ (δ.ᴬ γ) ;
                 ...
                 R   = λ {γc} γ {γᴬ} δ' → σ.R (δ.E γ) (δ.R γ δ') ;
