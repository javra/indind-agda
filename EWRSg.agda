...
record Con : Set₂ where
  field
    ...
    F   : ∀{γc}(γ : (E ᵃC) γc)(γᴬ : ᴬ){ρc : _ᵃc {zero} (Rc γ γᴬ)}(ρ : (R γ γᴬ ᵃC) ρc) → Set
...
--           (ρ : (R γ γᴬ ᵃC) ρc)(γᶠ : F γ γᴬ δ ρ)(τ : T γ γᴬ δ ρ) → ᴹ (sg γc γ δc δ) γᴬ
...
record TyS (Γ : Con) : Set₂ where
  ...
  field
    ...
    F   : ∀{γc}(γ : (Γ.E ᵃC) γc)(γᴬ : Γ.ᴬ){ρc : _ᵃc {zero} (Γ.Rc γ γᴬ)}(ρ : (Γ.R γ γᴬ ᵃC) ρc)
           (α : Set)(αᴬ : ᴬ γᴬ)(αᴿ : _ᵃS {zero} (R α αᴬ)) → Set
...
--           {ρc : _ᵃc {zero} (Γ.Rc γ γᴬ)}(ρ : (Γ.R γ γᴬ ᵃC) ρc)(γᶠ : Γ.F γ γᴬ δ ρ)(τ : Γ.T γ γᴬ δ ρ)
--           (α : Set)(αᴬ : ᴬ γᴬ)(αʷ : _ᵃS {zero} (w γc α))(αᴿ : _ᵃS {zero} (R α αᴬ))
--           (αᶠ : F γ γᴬ δ ρ α αᴬ αʷ αᴿ)(αᵗ : T γ γᴬ δ ρ α αᴬ αʷ αᴿ)
--           → ᴹ (Γ.m γ γᴬ δ ρ γᶠ τ) (sg γ δ α αʷ) αᴬ
...
record TmS (Γ : Con) (B : TyS Γ) : Set₂ where
  ...
  field
    ...
    F   : ∀{γc}(γ : (Γ.E ᵃC) γc)(γᴬ : Γ.ᴬ){ρc : _ᵃc {zero} (Γ.Rc γ γᴬ)}(ρ : (Γ.R γ γᴬ ᵃC) ρc)
           → Γ.F γ γᴬ  ρ → B.F γ γᴬ ρ ((E ᵃt) γc) (ᴬ γᴬ) ((R γ γᴬ ᵃt) ρc)
...
--    m   : ∀{γc}(γ : (Γ.E ᵃC) γc)(γᴬ : Γ.ᴬ){δc : _ᵃc {zero} (Γ.wc γ)}(δ : (Γ.wγ  ᵃC) δc)
--           {ρc : _ᵃc {zero} (Γ.Rc γ γᴬ)}(ρ : (Γ.R γ γᴬ ᵃC) ρc)
--           (γᶠ : Γ.F γ γᴬ δ ρ)(τ : Γ.T γ γᴬ δ ρ)
--           → coe (happly2 (B.ᴹ (B.Γ.m γ γᴬ δ ρ γᶠ τ)) (sg γ δ) (ᴬ γᴬ)) (ᴹ (Γ.m γ γᴬ δ ρ γᶠ τ))
--             ≡ B.m γ γᴬ δ ρ γᶠ τ ((E ᵃt) γc) (ᴬ γᴬ) ((w γ ᵃt) δc) ((R γ γᴬ ᵃt) ρc) (F γ γᴬ δ ρ γᶠ) (T γ γᴬ δ ρ τ)
...
record Sub (Γ : Con) (Δ : Con) : Set₂ where
  ...
  field
    ...
    F   : ∀{γc}(γ : _)(γᴬ : _){ρc}(ρ : _) → Γ.F {γc} γ γᴬ {ρc} ρ → Δ.F (E γ) (ᴬ γᴬ) (R γ ρ)
...
--    m   : ∀{γc}(γ : _)(γᴬ : _){δc}(δ : _){ρc} ρ γᶠ τ
--           → ᴹ (Γ.m {γc} γ γᴬ {δc} δ {ρc} ρ γᶠ τ)
--               ≡ coe (happly (Δ.ᴹ & sg γ δ) (ᴬ γᴬ) ⁻¹)
--                 (Δ.m (E γ) (ᴬ γᴬ) (w γ δ) (R γ ρ) (F γ γᴬ δ ρ γᶠ) (T γ γᴬ δ ρ τ))
...
Γ ▶S B = record { ᴬ   = Σ Γ.ᴬ B.ᴬ ;
                  ...
                  F   = λ { {γc , α} γ (γᴬ , αᴬ) {ρc , αᴿ} ρ → Γ.F γ γᴬ ρ × B.F γ γᴬ ρ α αᴬ αᴿ } ;
                  T   = λ { {γc , α} γ (γᴬ , αᴬ) {δc , αʷ} δ {ρc , αᴿ} ρ → Γ.T γ γᴬ δ ρ × B.T γ γᴬ δ ρ α αᴬ αʷ αᴿ }
                  {- m   = λ { {γc , α} γ (γᴬ , αᴬ) {δc , αʷ} δ {ρc , αᴿ} ρ (γᶠ , αᶠ) (τ , αᵗ)
                            → Γ.m γ γᴬ δ ρ γᶠ τ , B.m γ γᴬ δ ρ γᶠ τ α αᴬ αʷ αᴿ αᶠ αᵗ } -} }
  where
    module Γ = Con Γ
    module B = TyS B
...
Γ ▶P A = record { ᴬ   = Σ Γ.ᴬ A.ᴬ ;
                  ...
                  F   = λ { (γ , α) (γᴬ , αᴬ) (ρ , αᴿ) → Γ.F γ γᴬ ρ } ;
                  sg  = λ { γc (γ , α) δc (δ , ω) → Γ.sg γc γ δc δ , A.sg γ δ α ω } ;
                  T   = λ { (γ , α) (γᴬ , αᴬ) (δ , αʷ) (ρ , αᴿ) → Γ.T γ γᴬ δ ρ }
                  {- m   = λ { (γ , α) (γᴬ , αᴬ) (δ , αʷ) (ρ , αᴿ) γᶠ τ → Γ.m γ γᴬ δ ρ γᶠ τ , A.m γ γᴬ δ ρ γᶠ τ α αᴬ αʷ αᴿ } -} }
  where
    module Γ = Con Γ
    module A = TyP A
...
U {Γ} = record { ᴬ   = λ γ → Set ;
                 ...
                 F   = λ γ γᴬ ρ α αᴬ αᴿ
                         → (xᴱ : α)(xᴬ₀ xᴬ₁ : αᴬ)(r₀ : αᴿ xᴬ₀ xᴱ)(r₁ : αᴿ xᴬ₁ xᴱ) → xᴬ₀ ≡ xᴬ₁ ;
                 T   = λ γ γᴬ δ ρ α αᴬ αʷ αᴿ → (xᴱ : α)(xʷ : αʷ xᴱ) → Σ αᴬ λ xᴬ → αᴿ xᴬ xᴱ ;
                 sg  = λ γ δ α ω → Σ α ω
                 {- m   = λ { γ γᴬ δ ρ γᶠ τ α αᴬ αʷ αᴿ αᶠ αᵗ (xᴱ , xʷ) → ₁ (αᵗ xᴱ xʷ) } -} }
  where
    module Γ = Con Γ
...
El {Γ} a = record { ᴬ   = λ γᴬ → a.ᴬ γᴬ ;
                    ...
                    {- m   = λ γ γᴬ δ ρ γᶠ τ α αᴬ αʷ αᴿ
                            → let (αᴬ' , αᴿ') = a.t γ γᴬ δ ρ τ α αʷ in
                              let e = happly (a.m γ γᴬ δ ρ γᶠ τ) (α , αʷ) in
                              let e' = a.F γ γᴬ δ ρ γᶠ α αᴬ αᴬ' αʷ αᴿ αᴿ' in
                              let e'' = happly (coehapply2 (a.ᴹ (a.B.Γ.m γ γᴬ δ ρ γᶠ τ)) (a.sg γ δ)) (α , αʷ) in
                              lift (e'' ⁻¹ ◾ e ◾ e' ⁻¹) -} }
  where
    module Γ = Con Γ
    module a = TmS a
...
ΠS {Γ} a B = record { ᴬ   = λ γᴬ → (α : a.ᴬ γᴬ) → B.ᴬ (γᴬ , α) ;
                      ...
                      {- m   = λ {γc} γ γᴬ {δc} δ {ρc} ρ γᶠ τ π πᴬ πʷ πᴿ πᶠ πᵗ αᴬ
                              → let (xᴱ , xʷ) = coe (a.sg γ δ) αᴬ in
                                let (xᴬ , xᴿ) = a.T γ γᴬ δ ρ τ xᴱ xʷ in
                                let xᴬ' = a.ᴹ (Γ.m γ γᴬ δ ρ γᶠ τ) αᴬ in
                                let e' = happly (a.m γ γᴬ δ ρ γᶠ τ) (xᴱ , xʷ) in
                                let xᴿ' = coe (happly2 ((a.R γ γᴬ ᵃt) ρc) e' xᴱ ⁻¹) xᴿ in
                                let e'' = happly (coehapply2 (a.ᴹ (Γ.m γ γᴬ δ ρ γᶠ τ)) (a.sg γ δ)) (xᴱ , xʷ)
                                          ◾ a.ᴹ (Γ.m γ γᴬ δ ρ γᶠ τ) & coecoe⁻¹' (a.sg γ δ) αᴬ in
                                let e = a.F γ γᴬ δ ρ γᶠ xᴱ xᴬ xᴬ' xʷ xᴿ (coe (happly2 ((a.R γ γᴬ ᵃt) ρc) e'' xᴱ) xᴿ') in
                                coe
                                (Bᴹaux _ _ _ _ _ _ (coecoe⁻¹' (a.sg γ δ) αᴬ) e (Lift-irrel _ _) _
                                  (B.ᴬ & ,≡ refl e) (coe-coe _ _ (πᴬ xᴬ) ◾ apd πᴬ e))
                                (B.m {γc} (γ , xᴱ) (γᴬ , xᴬ) (δ , xʷ) {ρc} (ρ , xᴿ) γᶠ τ
...
ΠP {Γ} a B = record { ᴬ   = λ γᴬ → (α : a.ᴬ γᴬ) → B.ᴬ (γᴬ , α) ;
                      ...
                      {- m   = λ γ γᴬ δ {ρc} ρ γᶠ τ π πᴬ πʷ πᴿ αᴬ
                              → let (xᴱ , xʷ) = coe (a.sg γ δ) αᴬ in
                                let (xᴬ , xᴿ) = a.T γ γᴬ δ ρ τ xᴱ xʷ in
                                let xᴬ' = a.ᴹ (Γ.m γ γᴬ δ ρ γᶠ τ) αᴬ in
                                let xᴿ' = coe (happly2 ((a.R γ γᴬ ᵃt) ρc) (happly (a.m γ γᴬ δ ρ γᶠ τ) (xᴱ , xʷ)) xᴱ ⁻¹) xᴿ in
                                let e'' = happly (coehapply2 (a.ᴹ (Γ.m γ γᴬ δ ρ γᶠ τ)) (a.sg γ δ)) (xᴱ , xʷ)
                                          ◾ a.ᴹ (Γ.m γ γᴬ δ ρ γᶠ τ) & coecoe⁻¹' (a.sg γ δ) αᴬ in
                                let e = a.F γ γᴬ δ ρ γᶠ xᴱ xᴬ xᴬ' xʷ xᴿ (coe (happly2 ((a.R γ γᴬ ᵃt) ρc) e'' xᴱ) xᴿ') in
                                coe (Bᴹaux _ _ _ _ _ _ (coecoe⁻¹' (a.sg γ δ) αᴬ) e
                                  (Lift-irrel _ _) _ (B.ᴬ & ,≡ refl e) (coe-coe _ _ (πᴬ xᴬ) ◾ apd πᴬ e))
                                  (B.m (γ , xᴱ) (γᴬ , xᴬ) (δ , xʷ) (ρ , xᴿ) γᶠ τ (π xᴱ) (πᴬ xᴬ) (πʷ xᴱ xʷ) (πᴿ xᴬ xᴱ xᴿ)) -} }
  where
    module a = TmS a
    module B = TyP B
    module Γ = Con Γ
...
appS {Γ}{a}{B} f = record { ᴬ   = λ { (γ , α) → f.ᴬ γ α } ;
                            ...
                            sg  = λ { (γ , α) {δc} (δ , ω) →
                                        ...
                                        ◾ apd' (λ { (α , ω) → B.sg (γ , α) (δ , ω) _ ((f.w γ ᵃt) δc α) } )
                                            (coecoe⁻¹ (a.sg γ δ) (α , ω)) } ;
                            F   = λ { (γ , α) (γᴬ , αᴬ) (ρ , αᴿ) γᶠ → f.F γ γᴬ ρ γᶠ α αᴬ αᴿ } ;
                            T   = λ { (γ , α) (γᴬ , αᴬ) (δ , αʷ) (ρ , αᴿ) τ → f.T γ γᴬ δ ρ τ α αᴬ αʷ αᴿ }
                            {- m   = λ { {γc} (γ , α) (γᴬ , αᴬ) {δc} (δ , αʷ) {ρc} (ρ , αᴿ) γᶠ τ
                                      → let e = happly (f.m γ γᴬ δ ρ γᶠ τ) (coe (a.sg γ δ ⁻¹) (α , αʷ)) in
                                        --let ae = a.m γ γᴬ δ ρ γᶠ τ in
                                        coe-coe {!!} {!!} {!!}
                                        ◾ coe≡ e
                                        ◾ Bᵐcoecoe γ γᴬ δ ρ γᶠ τ (coe (a.sg γ δ) (coe (a.sg γ δ ⁻¹) (α , αʷ)))
                                            (α , αʷ) (αᴬ , αᴿ) (a.sg γ δ)
                                            {!!} } -} }
                                        -- essentially e
  where
...
Π̂S T B = record { ᴬ   = λ γᴬ → (τ : T) → TyS.ᴬ (B τ) γᴬ ;
                  ...
                  F   = λ γ γᴬ ρ π πᴬ πᴿ → (τ : T) → TyS.F (B τ) γ γᴬ ρ π (πᴬ τ) (πᴿ τ) ;
                  T   = λ γ γᴬ δ ρ π πᴬ πʷ πᴿ → (τ : T) → TyS.T (B τ) γ γᴬ δ ρ π (πᴬ τ) (πʷ τ) (πᴿ τ) ;
                  sg  = λ γ δ πᴱ πᴬ τ → TyS.sg (B τ) γ δ πᴱ (πᴬ τ)
                  {- m   = λ γ γᴬ δ ρ γᶠ τ' π πᴬ πʷ πᴿ πᶠ πᵗ τ
                          → TyS.m (B τ) γ γᴬ δ ρ γᶠ τ' π (πᴬ τ) (πʷ τ) (πᴿ τ) (πᶠ τ) (πᵗ τ) -} }
...
âppS {Γ}{T}{B} f τ = record { ᴬ  = λ γᴬ → f.ᴬ γᴬ τ ;
                              ...
                              F  = λ γ γᴬ ρ γᶠ → f.F γ γᴬ ρ γᶠ τ ;
                              T  = λ γ γᴬ δ ρ τ' → f.T γ γᴬ δ ρ τ' τ ;
                              sg = λ γ δ → happly (f.sg γ δ) τ
                              {- m  = λ γ γᴬ δ ρ γᶠ τ'
                                     → coe→ (f.ᴹ (Γ.m γ γᴬ δ ρ γᶠ τ')) τ _ _
                                       (ext (λ τ → happly2 (TyS.ᴹ (B τ) (f.B.Γ.m γ γᴬ δ ρ γᶠ τ'))
                                                   (happly (f.sg γ δ) τ) (f.ᴬ γᴬ τ))) ⁻¹
                                       ◾ happly (f.m γ γᴬ δ ρ γᶠ τ') τ -} }
  where
    module f = TmS f
    module Γ = Con Γ
...
_[_]TS B σ = record { ᴬ   = λ γ → B.ᴬ (σ.ᴬ γ) ;
                      ...
                      R   = λ T αᴬ → B.R T αᴬ ;
                      F   = λ γ γᴬ ρ α αᴬ αᴿ → B.F (σ.E γ) (σ.ᴬ γᴬ) (σ.R γ ρ) α αᴬ αᴿ ;
                      ...
                      {- m   = λ γ γᴬ δ ρ γᶠ τ α αᴬ αʷ αᴿ αᶠ αᵗ → coe {!!}
                                                                (B.m (σ.E γ) (σ.ᴬ γᴬ) (σ.w γ δ) (σ.R γ ρ) (σ.F γ γᴬ δ ρ γᶠ)
...
_[_]tS {Γ}{Δ}{A} a σ = record { ᴬ   = λ γ → a.ᴬ (σ.ᴬ γ) ;
                                ...
                                F   = λ γ γᴬ ρ γᶠ → a.F (σ.E γ) (σ.ᴬ γᴬ) (σ.R γ ρ) (σ.F γ γᴬ ρ γᶠ) ;
                                ...
                                sg  = λ {γc} γ {δc} δ →
                                              ...
                                              ◾ coe (A.ᴬ & (σ.sg γ δ ⁻¹))
                                                & (a.sg (σ.E γ) (σ.w γ δ))
                                {- m   = λ γ γᴬ δ ρ γᶠ τ
                                        → let m' = a.m (σ.E γ) (σ.ᴬ γᴬ) (σ.w γ δ) (σ.R γ ρ) (σ.F γ γᴬ δ ρ γᶠ) (σ.T γ γᴬ δ ρ τ) in
                                          {!!} -} }
  where
    module A = TyS A
    module a = TmS a
...
σ ,tS t = record { ᴬ   = λ γ → σ.ᴬ γ , t.ᴬ γ ;
                   ...
                   R   = λ γ δ → σ.R γ δ ;
                   F   = λ γ γᴬ ρ x → σ.F γ γᴬ ρ x , t.F γ γᴬ ρ x ;
...
< t >S = record
           ...
           ; R = λ γ γᴿ → γᴿ
           ; F = λ γ γᴬ ρ γᶠ → γᶠ , t.F γ γᴬ ρ γᶠ
           ; T = λ γ γᴬ δ ρ γᵗ → γᵗ , t.T γ γᴬ δ ρ γᵗ
