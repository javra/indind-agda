...
data Tm (Γc : SCon) : TyS → Set₁ where
  ...
  _$S_ : ∀{T B} → Tm Γc (ΠS T B) → (τ : T) → Tm Γc (B τ)

data TyP (Γc : SCon) : Set₁ where
  El   : Tm Γc U → TyP Γc
  Π^P   : (T : Set) → (T → TyP Γc) → TyP Γc
...
[id]T (Π^P T x) = Π^P T & ext λ α → [id]T (x α)
[id]T (El x)   = El & [id]t x
...
[][]T {Γ} {Δ} {Ω} (Π^P T B) δ γ = Π^P T & ext λ α → [][]T (B α) δ γ
...
Π^P[] : ∀{Γ Δ}{δ : Sub Γ Δ}{T}{A : T → TyP Δ} → (Π^P T A) [ δ ]T ≡ Π^P T λ α → A α [ δ ]T
Π^P[] = refl

$S[] : ∀{Γ Δ}{δ : Sub Γ Δ}{T}{B}{t : Tm Δ (ΠS T B)}{α} → (t $S α) [ δ ]t ≡ (t [ δ ]t) $S α
$S[] = refl
...
data TmP {Γc}(Γ : Con Γc) : TyP Γc → Set₁ where
  ...
  _$^P_ : ∀{T A} → TmP Γ (Π^P T A) → (τ : T) → TmP Γ (A τ)
...
[wkP]tP (σP ,P _)  (tP $^P τ)       = happly2 _$^P_ ([wkP]tP _ tP) τ
{-# REWRITE [wkP]tP #-}
...
[idP]tP {tP = tP $P sP}      = happly2 _$P_ ([idP]tP {tP = tP}) _
                               ◾ _$P_ tP & [idP]tP
[idP]tP {tP = tP $^P τ}       = happly2 _$^P_ [idP]tP τ
{-# REWRITE [idP]tP #-}
...
vsP[,P]tP {tP = tP $^P τ} {sP} {σP} = (λ tP → tP $^P τ)
