...
record TmP (Γ : Con) (A : TyP Γ) : Set₁ where
  ...
  module A = TyP A
  field
    E   : ∀{γc} → (Γ.E ᴬC) γc → (A.E ᴬP) γc
    w   : ∀{γc}{γ} → {δc : Γ.wc γc γ ᴬc} → (Γ.w γc γ ᴬC) δc → (A.w γ (E γ) ᴬP) δc
    Alg : (γ : Γ.Alg) → A.Alg γ
    sg  :  ∀{γc} → (γ : (Γ.E ᴬC) γc) → {δc : _} → (δ : (Γ.w γc γ ᴬC) δc) → Alg (Γ.sg γc γ δc δ) ≡ A.sg γ δ (E γ) (w δ)
